  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        final progress = widget.longest > 0 ? (widget.streak / widget.longest).clamp(0.0, 1.0) : 0.0;
        return SizedBox(
          width: 120, height: 120,
          child: CustomPaint(
            painter: _RingPainter(progress: progress * _animation.value, color: Theme.of(context).colorScheme.primary),
            child: Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  if (widget.streak >= 3) const Text('\u{1F525}', style: TextStyle(fontSize: 20)),
                  Text('${widget.streak}', style: const TextStyle(fontSize: 32, fontWeight: FontWeight.bold)),
