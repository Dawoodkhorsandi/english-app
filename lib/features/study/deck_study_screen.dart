...
class DeckStudyScreen extends ConsumerWidget {
  ...
  const DeckStudyScreen({super.key, required this.deckId, required this.deckName});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: cardsAsync.when(
        ...
        error: (e, s) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          const Icon(Icons.error_outline, size: 48),
          const SizedBox(height: 16),
          const Text('Could not load cards'),
          const SizedBox(height: 8),
          FilledButton(onPressed: () => ref.invalidate(deckStudyProvider(deckId)), child: const Text('Retry')),
        ])),
        data: (cards) {
          if (cards.isEmpty) return const Center(child: Text('No cards to study.'));
          return _SwipeSession(
            cards: cards.map((c) => _SwipeCard(
              front: c.term,
              sub: c.pronunciation.isNotEmpty ? c.pronunciation : null,
              back: [c.definition, if (c.example.isNotEmpty) 'Example: ${c.example}', if (c.persian.isNotEmpty) c.persian, if (c.mnemonic.isNotEmpty) 'Mnemonic: ${c.mnemonic}'].join('\n\n'),
              term: c.term,
            )).toList(),
            onAnswer: (card, known) async {
              final client = ref.read(apiClientProvider);
              await client.post('/api/decks/swipe', data: {'deck': deckId, 'term': card.term, 'known': known});
            },
...
class _SwipeCard {
  ...
  final String back;
  final String term;
  _SwipeCard({required this.front, this.sub, required this.back, required this.term});
}
...
class _SwipeSession extends StatefulWidget {
  ...
  const _SwipeSession({required this.cards, required this.onAnswer, this.doneText = 'Session complete!', this.onFinish});
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              const Spacer(),
              Text('$_knownCount known', style: const TextStyle(color: Colors.green)),
            ],
          ),
        ),
        Expanded(
          child: GestureDetector(
            ...
            child: Container(
              ...
              transform: Matrix4.translationValues(_dx, 0, 0)..rotateZ(_dx * 0.001),
