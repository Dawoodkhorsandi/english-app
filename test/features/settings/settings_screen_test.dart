...
AppSettings testSettings() => AppSettings(
  ...
  levelLabels: {'beginner': 'Beginner', 'intermediate': 'Intermediate', 'upper-intermediate': 'Upper-Intermediate', 'advanced': 'Advanced'},
  name: 'Test User',
  paused: false,
  interval: 60,
  toggles: {'tts': true, 'tips': true, 'quiz': true, 'idiom': true, 'collocation': true, 'story': true, 'review': true, 'daily_review': true, 'digest': false},
);

Widget wrapInSettingsApp({AppSettings? settings}) => ProviderScope(
  overrides: [
    apiClientProvider.overrideWithValue(_NoOpApiClient()),
    settingsProvider.overrideWith((ref) => Future.value(settings ?? testSettings())),
  ],
  ...
  group('SettingsScreen', () {
    ...
    testWidgets('toggle works', (tester) async {
      ...
      final ttsToggle = find.byWidgetPredicate(
        (w) => w is SwitchListTile && w.title != null && (w.title as Text?)?.data == 'Pronunciation audio',
      );
