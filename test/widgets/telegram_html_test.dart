...
import 'package:english_app/shared/widgets/telegram_html.dart';
...
void main() {
  testWidgets('renders plain text and decodes entities', (tester) async {
    await tester.pumpWidget(
      wrapInApp(const TelegramHtml('Tom &amp; Jerry &lt;3')),
    );
  ...
  testWidgets('strips tags into a single rich text run', (tester) async {
    await tester.pumpWidget(
      wrapInApp(const TelegramHtml('a <b>bold</b> word')),
    );
  ...
  testWidgets('spoiler is hidden until tapped', (tester) async {
    await tester.pumpWidget(
      wrapInApp(const TelegramHtml('secret: <tg-spoiler>hidden</tg-spoiler>')),
    );
